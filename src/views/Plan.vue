...
import { listSido, listGugun, getMountainList } from '../api/mountain';
...
const onChangeGugun = () => {
  ...
  param.value.sido_code = selectedSido.value;
  param.value.gugun_code = selectedGugun.value;
  getMountain();
};
    ...
    <div class="col-8 bg-light rounded-5 shadow">
      <div class="d-flex flex-column m-3 col-12">
        <div class="form d-flex flex-row col-2 mb-3">
          <select class="form-select bg-secondary fw-bold text-white" v-model="selectedSido" @change="onChangeSido(selectedSido)">
            <option selected disabled>시/도</option>
            <option v-for="sido in sidoList" :key="sido.value" :value="sido.value">
              {{ sido.text }}
            </option>
          </select>
          <select class="form-select bg-secondary fw-bold text-white" v-model="selectedGugun" @change="onChangeGugun()">
        ...
        <div class="mt-3 p-3">
          <div class="d-flex align-items-start">
            <div class="rounded-circle bg-warning d-flex justify-content-center align-items-center me-2" style="width: 22px; height: 22px">
              <i class="bi bi-suit-heart-fill text-danger"></i>
            </div>
            <p class="fw-bold fs-5">내 위시리스트</p>
          </div>
          <div class="d-flex">
            <MyWishList v-for="wishlist in wishlists" :key="wishlist" :wishlist="wishlist"></MyWishList>
          </div>
        </div>
      </div>
    </div>
    <div class="col-4 d-flex flex-column" :style="{ 'background-image': 'url(' + folderimg + ')' }" style="background-repeat: no-repeat">
      <div class="mt-4 ms-5">
        <div class="d-flex flex-row mt-1">
          <button type="button" class="btn text-primary ms-3 me-4" @click="closest()">가까운 순 <i class="bi bi-chevron-down"></i></button>
          <button type="button" class="btn text-primary ms-5" @click="high()">해발 고도 순 <i class="bi bi-chevron-down"></i></button>
        </div>
        <p class="fs-5 mt-3 fw-bold"><i class="bi bi-geo-alt-fill me-2 text-danger"></i>이런 산은 어때요?</p>
        <div class="d-flex flex-wrap d-flex flex-column">
          <RecommendCard v-for="item in items" :key="item.mntilistno" :RecommendCard="item"></RecommendCard>
        </div>
      </div>
    </div>
  </div>
