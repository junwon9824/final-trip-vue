<template>
    <div v-if="mountainDetail" class="content-container">
        ...
        <div class="additional-info">
            <MountainConqueror v-if="showMountainConqueror" :ranker="items"></MountainConqueror>
            <RecordCard v-if="showMountainConqueror" class="mt-5" :records="items"></RecordCard>
        </div>
    </div>
</template>
...
onMounted(async () => {
    ...
    try {
        ...
        try {
            const response2 = await axios.get(`http://localhost:80/mountain/getrank3`, {
                params: {
