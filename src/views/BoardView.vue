...
const getcommentlist = async () => {
  ...
  try {
    const response = await axios.get(
      "http://localhost:80/comments/allcomments",
      {
        params: {
          articleNo: boardId,
        },
...
<template>
  <div
    class="container bg-info d-flex align-items-center flex-column py-4 rounded-4 mt-5"
  >
  ...
  <div class="container d-flex align-items-center py-4 rounded-4 mt-5">
    ...
    <button
      class="btn bg-secondary fs-6 fw-bold text-white ms-3"
      type="button"
      @click="AddComment"
      id="commentbtn"
    >
